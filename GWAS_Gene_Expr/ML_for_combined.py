...
def train_ADNI(groups='CN_AD',features=1000,data_type = 'combined'):
    ...
    df_final.to_csv(os.path.join(results_path,'GWAS12_data_Dx_bl.csv'))

    df_final = pd.read_csv(os.path.join(results_path,'GWAS12_data_Dx_bl.csv'),na_values=["00"])
    ...
    # cat_columns_index = range(2+len(num_cols),final_N)
    ...
    plt.legend(loc="best")
    plt.grid(False)
    plt.savefig(os.path.join(results_path,'Grid_search_Using_'+str(STEP)+'_'+str(features)+'_'+str(data_type)+'_features_for:'+groups+'.png'))
    ...
    plt.show()
    plt.savefig(os.path.join(results_path,'ROC_for:'+groups+'_'+str(STEP)+'_'+str(features)+'_'+str(data_type)+'.png'))
    ...
    imp_df_sorted.to_csv(os.path.join(results_path,groups+'_Classification_ranked_'+str(STEP)+'_'+str(features)+'_'+str(data_type)+'_features.csv'))
